...
from config.users import USER1_NAME, USERS_PASSWORD, USER_FAKE_NAME, USERS_FAKE_PASSWORD
...
class TestAuth:
    ...
    def test_auth_009(self, page):
        """ Проверка отсутствия блокировки после 5 неудачных попыток """
        login_page = LoginPage(page)
        login_page.open()

        # Повторяем 5 неудачных попыток
        for _ in range(5):
            login_page.fill_username(USER1_NAME)
            login_page.fill_password(USERS_FAKE_PASSWORD)
            login_page.click_btn_login()
            # Проверяем, что отображается ошибка, а не блокировка
            login_page.check_error_with_msg(E_MSG_LOGIN)

        # 6-я попытка (проверка, что поле ввода всё ещё активно и система не заблокирована)
        login_page.fill_username(USER1_NAME)
        login_page.fill_password(USERS_PASSWORD)
        login_page.click_btn_login()
        ...
        login_page.expect_to_have_url("/inventory.html")
        inventory_page = InventoryPage(page)
        assert inventory_page.have_title("Products")
    ...
    def test_auth_010(self, page):
        ...
        # Убеждаемся, что мы внутри
        login_page.expect_to_have_url("/inventory.html")

        # Шаг 2: Перезагрузить страницу (F5)
        page.reload()

        # Шаг 3: Проверить, что сессия активна
        login_page.expect_to_have_url("/inventory.html")
        assert inventory_page.have_title("Products"), "Сессия потеряна после перезагрузки"
